...
# 학습용 말뭉치 데이터를 불러옴
corpus = read_file('final_ner.txt')
...
for t in corpus:
    ...
    for w in t:
        if len(w) == 4 :
            ...
            tagged_sentence.append((w[1] +' '+ w[2], w[4]))
            sentence.append(w[1] + ' ' + w[2])
            bio_tag.append(w[4])
        elif len(w) == 6 :
            tagged_sentence.append((w[1] +' '+ w[2] + ' ' + w[3], w[5]))
            sentence.append(w[1] + ' ' + w[2] + ' ' +  w[3])
            bio_tag.append(w[5])
        else :
            tagged_sentence.append((w[1] +' '+ w[2] + ' ' + w[3] + ' ' + w[4] + ' ' + w[5] + ' ' + w[6], w[8]))
            sentence.append((w[1] +' '+ w[2] + ' ' + w[3] + ' ' + w[4] + ' ' + w[5] + ' ' + w[6]))
            bio_tag.append(w[8])
...
print(p.word_index)
# 학습용 단어 시퀀스 생성
x_train = [p.get_wordidx_sequence(sent) for sent in sentences] # [14, 15, 16...]
y_train = tag_tokenizer.texts_to_sequences(tags)    #[2, 1, 1, ..]
# print(x_train)
